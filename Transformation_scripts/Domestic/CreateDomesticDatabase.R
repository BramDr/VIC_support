...
# Input
aqu.file = "Input/aquastat2.csv"
eur.file = "Input/env_wat_abs2.csv"
wwd.file = "Input/WaterUse.csv"
merge.file = "Input/mergeData.csv"
dom.out = "Output/DomPerCountry.csv"
...
colnames(eur.data) = c("process", "source", "unit", "countrycode",
                        paste0("with.", gsub(x = colnames(eur.data[5:35]), pattern = "X", replacement = "")))
eur.data = reshape(data = eur.data,
...
out.data = rbind(eur.data[,c("Country_number", "year", "with")], 
