...
# Calculate
for(year in years) {
  wind.file = list.files(path = force.dir, pattern = paste0("wind*", year), full.names = T)
  vp.file = list.files(path = force.dir, pattern = paste0("vp*", year), full.names = T)
  pr.file = list.files(path = force.dir, pattern = paste0("pr*", year), full.names = T)
  tas.min.file = list.files(path = force.dir, pattern = paste0("tas*min*", year), full.names = T)
  tas.max.file = list.files(path = force.dir, pattern = paste0("tas*max*", year), full.names = T)
  swdown.file = list.files(path = force.dir, pattern = paste0("swdown*", year), full.names = T)
  
  nc = nc_open(wind.file)
  dates = as.Date(nc$dim$time, origin = paste0(year, "-01-01"))
  nc_close(nc)
  ...
  for(x in 1:length(lons)){
    for(y in 1:length(lats)){
      ...
      file.out = paste0(dir.out, "force_WFDEI_", lats[y], "N_", lons[x], "E", ".", substr(year, 2, 4))
...
"
  ...
  ", lats[y], " ", lons[x]," -1 -1 -1           ! lat   lon   elevation   A   B"
      ...
      out.data = data.frame(station = station,
                            year = year, 
                            day = day,
                            irradiation = format(irradiation[x,y], nsmall = 2),
                            minimum.temperature = format(minimum.temperature[x,y], nsmall = 2),
                            maximum.temperature = format(maximum.temperature[x,y], nsmall = 2),
                            vapour.pressure = format(vapour.pressure[x,y], nsmall = 2),
                            wind = format(wind[x,y], nsmall = 2), 
                            precipitation = format(precipitation[x,y], nsmall = 2))
