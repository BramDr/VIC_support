...
# Input
dam.file = "Saves/GRanDcorrected.csv"
...
gdams.merge = merge(gdams.merge, aggregate(formula = cbind(USE_IRRI, USE_SUPP) ~ MODEL_LONG_DD + MODEL_LAT_DD, data = gdams, FUN = function(x){max(x != "", na.rm = T)}))
...
ldams.merge = merge(ldams.merge, aggregate(formula = cbind(USE_IRRI, USE_SUPP) ~ MODEL_LONG_DD + MODEL_LAT_DD, data = ldams, FUN = function(x){max(x != "", na.rm = T)}))
