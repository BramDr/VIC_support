...
# Input
cc.file = "Input/MIRCA2000_cropping_calendars_30min.txt"
...
cc.cell.paddy = aggregate(formula = cbind(Area.1,Area.2,Area.3,Area.4,Area.5,Area.6,Area.7,Area.8,Area.9,Area.10,Area.11,Area.12) ~ cell_ID + row + column, 
                                    data = cc[cc$crop == 3,], FUN = sum)
cc.cell.paddy$paddyMax = apply(X = cc.cell.paddy[,paste0("Area.", 1:12)], MARGIN = 1, FUN = max)

cc.cell.irr = aggregate(formula = cbind(Area.1,Area.2,Area.3,Area.4,Area.5,Area.6,Area.7,Area.8,Area.9,Area.10,Area.11,Area.12) ~ cell_ID + row + column, 
                          data = cc[cc$crop %in% c(1:2, 4:26),], FUN = sum)
cc.cell.irr$irrMax = apply(X = cc.cell.irr[,paste0("Area.", 1:12)], MARGIN = 1, FUN = max)

cc.cell.rain = aggregate(formula = cbind(Area.1,Area.2,Area.3,Area.4,Area.5,Area.6,Area.7,Area.8,Area.9,Area.10,Area.11,Area.12) ~ cell_ID + row + column, 
                          data = cc[cc$crop %in% c(27:52),], FUN = sum)
cc.cell.rain$rainMax = apply(X = cc.cell.rain[,paste0("Area.", 1:12)], MARGIN = 1, FUN = max)

cc.merge = cc
cc.merge = join(cc.merge, cc.cell.paddy[,c("cell_ID", "paddyMax")], all.x = T)
cc.merge = join(cc.merge, cc.cell.irr[,c("cell_ID", "irrMax")], all.x = T)
cc.merge = join(cc.merge, cc.cell.rain[,c("cell_ID", "rainMax")], all.x = T)
cc.merge$areaMax = NA
