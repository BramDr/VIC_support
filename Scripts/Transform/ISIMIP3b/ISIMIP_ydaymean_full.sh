...
for variable in $variables; do
  for in_model in $in_models; do
    ...
    out_file2="./Saves/"$variable"_daily_"$in_model"_ydaymean_2020_2050.nc"
    out_file3="./Saves/"$variable"_daily_"$in_model"_ydaymean_2070_2100.nc"
    
    in_files1=""
    for in_year in $in_years1; do
      in_file=$(find $in_dir -type f -name "*"$variable"_daily_"$in_model"*"$in_year".nc")
    ...
    if [ "$in_files1" != "" ]; then
      ...
      out_file1="./Saves/"$variable"_daily_"$in_model"_ydaymean_1970_2000.nc"
