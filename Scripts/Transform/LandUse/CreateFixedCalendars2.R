...
# Input
fao.file <- "../../../Data/Primary/FAO/Allen1998/FAO_crop_characteristics2.csv"
cc.file <- "../../../Data/Transformed/LandUse/subcropCalendar_30min_global.csv"
fixed.out <- "../../../Data/Transformed/LandUse/subcropCalendar_fixed2_30min_global.csv"
...
add.fixed <- function(x, columns) {
  ...
  root_depth.3 <- as.numeric(fao$root_depth.3[row])
  root_frac.1 <- as.numeric(fao$root_frac.1[row])
  root_frac.2 <- as.numeric(fao$root_frac.2[row])
  root_frac.3 <- as.numeric(fao$root_frac.3[row])
  ...
  out <- c(root_depth.1, root_depth.2, root_depth.3, root_frac.1, root_frac.2, root_frac.3, RGL, rarc, rad_atten, overstory, trunk_ratio, wind_atten, wind_h, rmin, fcanopy)
  return(out)
}
...
colnames(fixed) <- c("root_depth.1", "root_depth.2", "root_depth.3", "root_frac.1", "root_frac.2", "root_frac.3", "RGL", "rarc", "rad_atten", "overstory", "trunk_ratio", "wind_atten", "wind_h", "rmin", "fcanopy2")
