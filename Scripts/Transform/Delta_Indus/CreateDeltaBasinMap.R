...
delta.file <- "../../../Data/Transformed/Delta/delta_5min_Indus.RDS"
downstream.file <- "../../../Data/Transformed/Routing/downstream_5min_Indus.RDS"
basin.out <- "../../../Data/Transformed/Delta/deltaBasins_5min_Indus.RDS"
delta.out <- "../../../Data/Transformed/Delta/deltaAdj_5min_Indus.RDS"
...
for (delta.id in unique(delta.basin$delta)) {
  ...
  rows <- which(delta.basin.sel$count > 150)
  if (length(rows) <= 1) {
    next
  }
  ...
  for (j in 2:nrow(delta.basin.major)) {
    ...
    if(nrow(delta.basin.minor) <= 0){
      next
    }
    
    for (k in 1:nrow(delta.basin.minor)) {
      new.dist <- get.distance(
        delta.basin.major$x[j], delta.basin.major$y[j],
        delta.basin.minor$x[k], delta.basin.minor$y[k]
      )
      orig.dist <- get.distance(
        delta.basin.major$x[1], delta.basin.major$y[1],
        delta.basin.minor$x[k], delta.basin.minor$y[k]
      )
