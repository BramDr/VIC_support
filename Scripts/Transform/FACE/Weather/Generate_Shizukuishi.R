...
# Input
wfdei.file = "./Saves/weather_Shizukuishi_WFDEI.RDS"
observed.file = "./Saves/weather_Shizukuishi.RDS"
vic.out <- "../../../../Data/VIC/Forcing/FACE/Shizukuishi/forcing_daily.nc"
years = 1998:2000
...
for(year in years){
  ...
  tas = observed$mean.air.temp[date.sel]
  swdown = observed$solar.rad[date.sel]
  wind = observed$wind.speed[date.sel]
  pr = observed$Pr[date.sel]
