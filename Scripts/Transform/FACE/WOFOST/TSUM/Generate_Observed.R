...
weather.dir <- "../../Weather"
tsum.out <- "./Saves/tsum_observed.RDS"

# Load
phenology = readRDS(phenology.file)
weather.files <- list.files(path = weather.dir, pattern = "observed.*nc", full.names = T)
...
## Get temperature
temp = data.frame(time = seq.Date(from = as.Date("1992-01-01"), 
                                  to = as.Date("1997-12-31"), 
                                  by = "day"),
                  temp = NA)
...
for(treatment in phenology$treatment){
  time.min = temp$time >= phenology$plant[phenology$treatment == treatment]
  time.max = temp$time <= phenology$maturity[phenology$treatment == treatment]
  ...
  Isow <- 1
  ## Emergence
  Iem = which(time.sel == phenology$emergence[phenology$treatment == treatment]) - 1
  Tair.em <- Tair.sel[(Isow + 1):Iem] # Start 1 day after sowing (WOFOST standard)
  Tair.em <- Tair.em - Tbase
  Tair.em[Tair.em < 0] <- 0
  Tair.em[Tair.em > Tmax - Tbase] <- Tmax - Tbase
  Tair.em <- cumsum(Tair.em)
  tem = Tair.em[length(Tair.em)]
  ## TSUM 1
  Itsum1 = which(time.sel == phenology$anthesis[phenology$treatment == treatment])
  Tair.tsum1 <- Tair.sel[(Iem + 1):Itsum1] # Start 1 day after emergence (WOFOST standard)
  Tair.tsum1 <- afgen(as.matrix(Ttable), Tair.tsum1)
  Tair.tsum1 <- cumsum(Tair.tsum1)
  tsum1 = Tair.tsum1[length(Tair.tsum1)]
  ## TSUM 2
  Itsum2 = which(time.sel == phenology$maturity[phenology$treatment == treatment]) - 1
