...
vic.out <- "../../../../Data/VIC/Parameters/global/soil_params_VlietFactor_global.nc"

# Load
source(map.support.file)
source(generate.support.file)

expt.fact <- readRDS(expt.factor.file)
ksat.fact <- readRDS(ksat.factor.file)
...
expt.fill <- fillMap(expt * expt.fact, na.map, getNearestMean)
ksat.fill <- fillMap(ksat * ksat.fact, na.map, getNearestMean)
