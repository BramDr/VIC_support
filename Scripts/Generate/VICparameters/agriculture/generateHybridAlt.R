...
parameter.file <- "Hybrid/Saves/parameters_30min_global.RDS"
vegetation.file <- "../../../../Data/VIC/Parameters/global/vegetation_params_VlietAlt_global.nc"
vegetation.out <- "../../../../Data/VIC/Parameters/global/vegetation_params_MIRCAhybridAlt_global.nc"
...
parameter.out.tmp = gsub(x = parameter.file, pattern = "parameters_", replacement = paste0("parameters", "Irrigated", "_"))
irr <- readRDS(parameter.out.tmp)
parameter.out.tmp = gsub(x = parameter.file, pattern = "parameters_", replacement = paste0("parameters", "Paddy", "_"))
paddy <- readRDS(parameter.out.tmp)
parameter.out.tmp = gsub(x = parameter.file, pattern = "parameters_", replacement = paste0("parameters", "Rainfed", "_"))
rain <- readRDS(parameter.out.tmp)

for(i in 1:nrow(split)){
  parameter.out.tmp = gsub(x = parameter.file, pattern = "parameters_", 
