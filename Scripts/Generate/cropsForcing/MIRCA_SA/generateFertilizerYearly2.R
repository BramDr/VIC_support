...
out.dir <- "../../../../Data/VIC/Forcing/global/MIRCA_SA/fertilizer2_yearly/fertilizer2_yearly_"
years = 1979:2016
...
fert.dvs.file = list.files(path = fert.dir, pattern = "fertilizer2DVS", full.names = T)
fert.n.file = list.files(path = fert.dir, pattern = "fertilizer2N", full.names = T)
fert.p.file = list.files(path = fert.dir, pattern = "fertilizer2P", full.names = T)
fert.k.file = list.files(path = fert.dir, pattern = "fertilizer2K", full.names = T)
...
for(i in 1:nrow(crops)){
  ...
  fert.dvs.fill = fillMap(fert.dvs[,,i,,], na.map, getNearestZero)
  fert.n.fill = fillMap(fert.n[,,i,,], na.map, getNearestZero)
  fert.p.fill = fillMap(fert.p[,,i,,], na.map, getNearestZero)
  fert.k.fill = fillMap(fert.k[,,i,,], na.map, getNearestZero)
