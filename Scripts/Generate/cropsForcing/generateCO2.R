...
# Input
co2.file = "../../../Data/Primary/NOAA/co2_annmean_mlo.csv"
out.dir <- "../../../Data/VIC/Forcing/global/co2_yearly/co2_yearly_"

# Load
co2 = read.table(co2.file, sep = ",", header = T)
...
for (z in 1:nrow(co2)) {
  ...
  data <- array(co2$mean[z], dim = c(lon.dim$len, lat.dim$len, time.dim$len))
