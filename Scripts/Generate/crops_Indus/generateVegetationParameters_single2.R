...
vegetation.file <- "../../../Data/VIC/Parameters/Indus_5min/vegetation_params_Mirca_Indus.nc"
vegetation.out <- "../../../Data/VIC/Parameters/Indus_5min/single/vegetation_params_single_Indus.nc"
...
Nveg <- array(1, dim = c(dim(mask)))
cv <- array(1.0, dim = c(dim(mask), veg_class))
root.fract <- array(0, dim = c(dim(mask), veg_class, nroot))
root.depth <- array(0, dim = c(dim(mask), veg_class, nroot))
lai <- array(5, dim = c(dim(mask), veg_class, nmonth))
displacement <- array(1.5 * 0.67, dim = c(dim(mask), veg_class, nmonth))
veg.rough <- array(1.5 * 0.123, dim = c(dim(mask), veg_class, nmonth))
albedo <- array(0.1, dim = c(dim(mask), veg_class, nmonth))
fcanopy <- array(1, dim = c(dim(mask), veg_class, nmonth))
overstory <- array(0, dim = c(dim(mask), veg_class))
rarc <- array(2, dim = c(dim(mask), veg_class))
rmin <- array(80, dim = c(dim(mask), veg_class))
rgl <- array(100, dim = c(dim(mask), veg_class))
sol.atten <- array(0.5, dim = c(dim(mask), veg_class))
wind.atten <- array(0.5, dim = c(dim(mask), veg_class))
trunk.ratio <- array(0.2, dim = c(dim(mask), veg_class))
wind.h <- array(10, dim = c(dim(mask), veg_class))
...
#system(command = paste0("ncks -h -x -v ", paste0(c(veg.vars, "veg_class", "fcanopy"), collapse = ","), " ", vegetation.file, " -O ", vegetation.out))
...
var.test = ncvar_def(
  name = "test", units = "test",
  dim = list(dim.lon, dim.lat, dim.month, dim.root_zone),
  longname = "test variable to include dimensions, ignore!"
)
nc = nc_create(filename = vegetation.out, vars = list(var.test), force_v4 = T)
nc_close(nc)
