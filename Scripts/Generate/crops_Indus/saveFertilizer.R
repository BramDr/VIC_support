...
disaggregate.maps = function(orig.map, new.lons, new.lats){
  ...
  for(c in 1:dim(orig.map)[3]){
    for(f in 1:dim(orig.map)[4]){
      for(x in 1:dim(new.map)[1]){
        x.orig = floor((x - 1) / factor) + 1
        new.diss = rep(orig.map[x.orig,,c,f], each = factor)
        new.map[x,,c,f] = new.diss
      }
    }
  }
...
# Calculate
fertilizer.n <- array(0, dim = c(dim(man.n.sel)[1:2], nrow(crops), nferttimes))
fertilizer.p <- array(0, dim = c(dim(man.n.sel)[1:2], nrow(crops), nferttimes))
fertilizer.k <- array(0, dim = c(dim(man.n.sel)[1:2], nrow(crops), nferttimes))
fertilizer.dvs <- array(0, dim = c(dim(man.n.sel)[1:2], nrow(crops), nferttimes))
...
for (i in 1:nrow(crops)) {
  ...
  fert.n.year = (fert.n.sel[,,nc.years == 2000] + man.n.sel[,,nc.years == 2000])
...
image.plot(fertilizer.n.adj[,,1,1])
