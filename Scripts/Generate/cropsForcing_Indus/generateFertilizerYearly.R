...
out.dir <- "../../../../Data/VIC/Forcing/Indus_5min/fertilizer_yearly/fertilizer_yearly_"
...
for (z in 1:length(years)) {
  ...
  fert.dvs.fill <- fillMap(fert.dvs[,,z,,], na.map, getNearestZero)
  fert.n.fill <- fillMap(fert.n[,,z,,], na.map, getNearestZero)
  fert.p.fill <- fillMap(fert.p[,,z,,], na.map, getNearestZero)
  fert.k.fill <- fillMap(fert.k[,,z,,], na.map, getNearestZero)
  ...
  for (i in 1:nrow(crops)) {
    ...
    ncvar_put(nc, var.DVS_point, fert.dvs.fill[, , i, ], start = c(1, 1, i, 1, 1), count = c(-1, -1, 1, -1, -1))
    ncvar_put(nc, var.N_amount, fert.n.fill[, , i, ], start = c(1, 1, i, 1, 1), count = c(-1, -1, 1, -1, -1))
    ncvar_put(nc, var.P_amount, fert.p.fill[, , i, ], start = c(1, 1, i, 1, 1), count = c(-1, -1, 1, -1, -1))
    ncvar_put(nc, var.K_amount, fert.k.fill[, , i, ], start = c(1, 1, i, 1, 1), count = c(-1, -1, 1, -1, -1))
  }
  nc_close(nc)
}
