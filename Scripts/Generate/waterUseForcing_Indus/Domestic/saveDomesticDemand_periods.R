library(fields)
library(ncdf4)
rm(list = ls())

# Input
iso.file <- "../../../../Data/Primary/ISO3166/ISO3166_dev.csv"
with.file <- "../../../../Data/Transformed/Domestic/domesticWithdrawal_country.csv"
tmon.file <- "../../../../Data/Transformed/ERA5/tas_daily_ERA5adj_ymonmean_1980_2010.nc"
tyear.file <- "../../../../Data/Transformed/ERA5/tas_daily_ERA5adj_yearmean_1980_2010.nc"
tyear.max.file <- "../../../../Data/Transformed/ERA5/tas_daily_ERA5adj_ymonmeanmax_1980_2010.nc"
tyear.min.file <- "../../../../Data/Transformed/ERA5/tas_daily_ERA5adj_ymonmeanmin_1980_2010.nc"
area.file <- "../../../../Data/Transformed/Routing/area_5min_Indus.RDS"
hist.pop.file <- "../../../../Data/Transformed/Population/population_historical_extended_5min_Indus.RDS"
ssp1.pop.file <- "../../../../Data/Transformed/Population/population_ssp1_extended_5min_Indus.RDS"
ssp3.pop.file <- "../../../../Data/Transformed/Population/population_ssp3_extended_5min_Indus.RDS"
dom.out <- "Saves/domesticDemand_periods_5min_Indus.RDS"
periods = data.frame(scenario = c("historical", "ssp126", "ssp126", "ssp370", "ssp370", "ssp585", "ssp585"),
                     syear = c(1970, 2020, 2070, 2020, 2070, 2020, 2070),
                     eyear = c(2000, 2050, 2100, 2050, 2100, 2050, 2100),
                     stringsAsFactors = F)

# Load
iso <- read.csv(file = iso.file, stringsAsFactors = F)
with <- read.csv(with.file, stringsAsFactors = F)
area <- readRDS(area.file)

nc <- nc_open(tmon.file)
tmon <- ncvar_get(nc, "tas")
nc_close(nc)
nc <- nc_open(tyear.file)
tyear <- ncvar_get(nc, "tas")
nc_close(nc)
nc <- nc_open(tyear.max.file)
tyear.max <- ncvar_get(nc, "tas")
nc_close(nc)
nc <- nc_open(tyear.min.file)
tyear.min <- ncvar_get(nc, "tas")
nc_close(nc)

# Calculate
dom.int <- with[with$Country_number == iso$Country_number[iso$Country == "Pakistan"],]$WithPc

dom.dem.data = list()
i = 1
for(i in 1:nrow(periods)){
  period.pattern = paste0(periods$scenario[i], "_", periods$syear[i], "_", periods$eyear[i])
  print(period.pattern)
  
  if(periods$scenario[i] == "historical"){
    pop.file = hist.pop.file
  } else if(periods$scenario[i] == "ssp126"){
    pop.file = ssp1.pop.file
  } else if(periods$scenario[i] == "ssp370"){
    pop.file = ssp3.pop.file
  } else if(periods$scenario[i] == "ssp585"){
    pop.file = ssp1.pop.file
  }
  pop = readRDS(pop.file)
  pop.years = as.numeric(dimnames(pop)[[3]])
  pop = pop[,,which(pop.years == periods$eyear[i])]
  
  dom.dem <- array(0, dim = c(dim(pop), 12))
  m = 1
  for (m in 1:12) {
    frac <- 1 + (tmon[, , m] - tyear) / (tyear.max - tyear.min) * 0.45
    dom.dem[, , m] <- dom.int * pop / 365 / area * 1e3 * frac # m3 cap-1 y-1 to mm day-1
  }
  dom.dem.mean = apply(X = dom.dem, MARGIN = c(1,2), FUN = mean)
  image.plot(dom.dem.mean, main = period.pattern)
  
  dom.dem.data[[period.pattern]] = dom.dem
}

# Save
dir.create(dirname(dom.out))
saveRDS(dom.dem.data, dom.out)
