...
fit.file <- "Saves/domestic_country_fitting_total.csv"
fit.out <- "Saves/domesticCoef_country_total.csv"

# Load
iso <- read.csv(file = iso.file, stringsAsFactors = F)
data.fit <- read.csv(fit.file, stringsAsFactors = F)
...
fit.global <- nls(
  formula = lWithPc ~ log(10^(lPar + (u - lPar) / (1 + exp(-f * (lGdpPc - o)))) * Int^(Year - 1980), 10),
  data = data.fit,
  start = list(
    u = max(data.fit$lWithPc),
    o = 3,
    f = 3
  ),
...
for (region in unique(data.fit$Subregion)) {
  ...
  try(fit <- nls(
    formula = lWithPc ~ log(10^(lPar + (uPar - lPar) / (1 + exp(-f * (lGdpPc - o)))) * Int^(Year - 1980), 10),
    data = region.data,
    start = list(
      o = coefficients(fit.global)[["o"]],
...
data.fit.s <- aggregate(
  formula = cbind(uPar, lPar, goPar, gfPar, soPar, sfPar, roPar, rfPar) ~ Subregion_number,
  data = data.fit, FUN = mean, na.rm = T
)
data.fit.r <- aggregate(
  formula = cbind(uPar, lPar, goPar, gfPar, soPar, sfPar, roPar, rfPar) ~ Region_number,
  data = data.fit, FUN = mean, na.rm = T
)
...
write.csv(x = coef, file = fit.out, row.names = F)
