...
loc.paddy <- aggregate(x = cc[cc$crop == 3, c("lon", "lat", "row", "column")], 
                       by = list(cc$cell_ID[cc$crop == 3]), FUN = mean)
loc.irr <- aggregate(x = cc[cc$crop %in% c(1:2, 4:26), c("lon", "lat", "row", "column")], 
                     by = list(cc$cell_ID[cc$crop %in% c(1:2, 4:26)]), FUN = mean)
loc.rain <- aggregate(x = cc[cc$crop %in% c(27:52), c("lon", "lat", "row", "column")], 
