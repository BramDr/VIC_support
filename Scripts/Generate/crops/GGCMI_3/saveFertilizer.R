...
# Calculate
fertilizer.n = array(0, dim = c(length(lons), length(lats), nrow(crops), nferts))
fertilizer.p = array(0, dim = c(length(lons), length(lats), nrow(crops), nferts))
fertilizer.k = array(0, dim = c(length(lons), length(lats), nrow(crops), nferts))
fertilizer.dvs = array(0, dim = c(length(lons), length(lats), nrow(crops), nferts))
...
for(i in 1:nrow(crops)){
  ...
  fertilizer.n[,,i,1] = (fert.n[,dim(fert.n)[2]:1,years == 2000] + man.n[,dim(man.n)[2]:1,years == 2000]) * 0.2
  fertilizer.n[,,i,2] = (fert.n[,dim(fert.n)[2]:1,years == 2000] + man.n[,dim(man.n)[2]:1,years == 2000]) * 0.8
}
