...
domain.out <- "../../../../Data/VIC/Parameters/global/GGCMI_3/domain_global.nc"
...
for(i in 1:nrow(crops)){
  ...
  domain.out.tmp = gsub(x = domain.out, pattern = "domain_", replacement = paste0("domain_", crops$name[i], "_", crops$water[i], "_"))
  ...
  mask = mask.t & !is.na(Ncrop[,,i]) & Ncrop[,,i] > 0 & !is.na(tsum1[,,i]) & !is.na(tsum2[,,i])
  image.plot(mask)
