  ...
  for(x in 1:dim(cc.crop)[1]){
    for(y in 1:dim(cc.crop)[2]){
      ...
      if(plant.sel < harvest.sel) {
        Tair.sel = Tair[x,y,plant.sel:harvest.sel] - 273.15 # K to celsius
      } else if(plant.sel > harvest.sel){
        Tair.sel = c(Tair[x,y,plant.sel:365], Tair[x,y,1:harvest.sel]) - 273.15 # K to celsius
      }
      ...
      #Tsums = calcPhaseDuration(Tair.dev, crops$name[i])
      #Tsums = calcPhaseGDD(Tair.dev[Idev], crops$name[i], lats[y])
