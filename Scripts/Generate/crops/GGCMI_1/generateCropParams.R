...
crop.out = "../../../../Data/VIC/Parameters/global/GGCMI_1/crop_params_global.nc"
...
for(i in 1:nrow(crops)){
  ...
  if(crops$name[i] != "wheat" && crops$name[i] != "maize" && crops$name[i] != "soybean" && crops$name[i] != "rice"){
    next
  }
  ...
  min.n.filled <- fillMap(map = min.n, na.map =na.map, nearest.function = getNearestMean)
  min.p.filled <- fillMap(map = min.p, na.map =na.map, nearest.function = getNearestMean)
  min.k.filled <- fillMap(map = min.k, na.map =na.map, nearest.function = getNearestMean)
  rec.n.filled <- fillMap(map = rec.n, na.map =na.map, nearest.function = getNearestMean)
  rec.p.filled <- fillMap(map = rec.p, na.map =na.map, nearest.function = getNearestMean)
  rec.k.filled <- fillMap(map = rec.k, na.map =na.map, nearest.function = getNearestMean)
