...
crop.out <- "../../../../Data/VIC/Parameters/global/CO2Impact_SA/crop_params_global.nc"
...
for (i in 1:nrow(crops)) {
  ...
  carbon.filled <- fillMap(map = carbon[, , i], na.map = na.map, nearest.function = getNearestMean)
  ph.filled <- fillMap(map = ph[, , i], na.map = na.map, nearest.function = getNearestMean)
